use buttplug::{
  core::{
    connector::{transport::ButtplugStreamTransport, ButtplugRemoteServerConnector}, message::
      serializer::{
        ButtplugSerializedMessage, ButtplugServerJSONSerializer,
      }
    
  },
  server::{device::ServerDeviceManager, ButtplugServerBuilder}, util::stream::convert_broadcast_receiver_to_stream,
};
use std::sync::Arc;
use tokio::sync::{broadcast, mpsc::{self, Sender}};
...
impl BackdoorServer {
  pub fn new(device_manager: Arc<ServerDeviceManager>) -> Self {
    let server = ButtplugRemoteServer::new(ButtplugServerBuilder::with_shared_device_manager(device_manager.clone()).name("Intiface Backdoor Server").finish().unwrap());
  ...
  pub async fn parse_message(&self, msg: &str) {
    self.sender.send(ButtplugSerializedMessage::Text(msg.to_owned())).await.unwrap();
  }
}
